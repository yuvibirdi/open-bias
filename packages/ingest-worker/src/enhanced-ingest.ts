...
/**
 * Enhanced similarity calculation using multiple techniques
 */
function calculateSimilarity(article1: { title: string; summary?: string }, article2: { title: string; summary?: string }): number {
  const title1 = article1.title.toLowerCase();
  const title2 = article2.title.toLowerCase();
  
  // 1. Title similarity using Levenshtein distance
  const maxLen = Math.max(title1.length, title2.length);
  const titleSimilarity = maxLen > 0 ? 1 - (levenshteinDistance(title1, title2) / maxLen) : 0;
  ...
  const wordOverlapSimilarity = union.size > 0 ? intersection.size / union.size : 0;
  
  // 3. TF-IDF similarity (if summaries available)
  let tfidfSimilarity = 0;
  if (article1.summary && article2.summary && 
      article1.summary.length > ENHANCED_CONFIG.MIN_SUMMARY_LENGTH && 
      article2.summary.length > ENHANCED_CONFIG.MIN_SUMMARY_LENGTH) {
    
    const tfidf = new TfIdf();
    tfidf.addDocument(`${title1} ${article1.summary.toLowerCase()}`);
    tfidf.addDocument(`${title2} ${article2.summary.toLowerCase()}`);
    ...
    if (terms1.length > 0 && terms2.length > 0) {
      ...
      tfidfSimilarity = cosineSimilarity(vector1, vector2);
    }
  }
  ...
  return (titleSimilarity * weights.title) + 
         (wordOverlapSimilarity * weights.wordOverlap) + 
